...
def load_rectangle_area(csv_path):
    """
    rectangles.csv を読み込み、四角形の4頂点を返す
    戻り値:
        rect_points: np.array shape=(4, 2), dtype=np.float32
                     [[x1, y1],
                      [x2, y2],
                      [x3, y3],
                      [x4, y4]]
    ...
    with open(csv_path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row_idx, row in enumerate(reader, start=1):
            ...
            if len(row) < 2:
                print(f"[WARNING] row {row_idx} is invalid: {row}")
                continue
            ...
            try:
                ...
                print(f"[WARNING] row {row_idx} has non-numeric values: {row}")
                continue

    if len(points) != 4:
        print(f"[ERROR] rectangle needs 4 points, but got {len(points)} points.")
        return np.empty((0, 2), dtype=np.float32)

    return np.array(points, dtype=np.float32)


def draw_danger_rectangle(image, rect_points, color=(0, 255, 255), thickness=3):
    """
    危険エリアの四角形を動画上に描画する
    rect_points:
        np.array shape=(4, 2)
...
def is_point_in_danger_rectangle(point, rect_points, image_shape):
    """
    1点が危険エリア四角形内にあるか判定する
    point:
        [x, y] のピクセル座標
    rect_points:
        np.array shape=(4, 2)
...
def ankle_in_danger_rectangle(land, conf, rect_points, image_shape, conf_th=0.2):
    """
    MoveNet / COCO17 の足首点(15,16)が危険エリア内に入ったか判定する
    どちらか一方の足首が入っていれば True
    """
    lands = np.array(land).reshape(-1, 2)
    conf = np.array(conf).reshape(-1)

    for ankle_idx in [15, 16]:
        if ankle_idx < len(lands) and ankle_idx < len(conf):
            if conf[ankle_idx] >= conf_th:
                if is_point_in_danger_rectangle(lands[ankle_idx], rect_points, image_shape):
                    return True
...
def exec_motion_fun(motion_queue, result_queue, csv_name):
    ...
    # 危険エリア（四角形）の読出し
    danger_rect = load_rectangle_area(csv_name)
    print("danger_rect =", danger_rect)
    ...
    while True:
        ...
        # 危険エリア（四角形）を描画
        draw_image = draw_danger_rectangle(draw_image, danger_rect, color=(0, 255, 255), thickness=3)

        # 登録している人がロスされていなければ、判定と描画を行う
        for land_pose, conf_pose in zip(joints, scores_list):
            # エリア内判定（足首: 15, 16）
            flag = ankle_in_danger_rectangle(land_pose, conf_pose, danger_rect, draw_image.shape)

            # 骨格描画(land poseは0~1にしてください)
            draw_image = confplot(draw_image, land_pose, conf_pose, flag)

        result_queue.put(draw_image)
