...
class SimpleNet(torch.nn.Module):
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False, evaluation=False):
        ...
        for i, feat in enumerate(features):
            if len(feat.shape) == 3:
                B, L, C = feat.shape
                features[i] = (
                    feat.reshape(B, int(math.sqrt(L)), int(math.sqrt(L)), C)
                    .permute(0, 3, 1, 2)
                )
    ...
    def test(self, training_data, test_data, save_segmentation_images):
        ...
        # ---- image-level AUROC (main metric for your use-case) ----
        auroc = metrics.compute_imagewise_retrieval_metrics(scores, anomaly_labels)["auroc"]

        # ---- pixel-level AUROC (optional; will be NaN for your dataset) ----
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        if masks_gt is not None and len(masks_gt) > 0:
            ...
            # PRO requires valid masks; returns NaN safely if not possible
            try:
    ...
    def _predict_dataloader(self, dataloader, prefix):
        ...
        with tqdm.tqdm(dataloader, desc="Inferring...", leave=False) as data_iterator:
            for data in data_iterator:
                ...
                for score, mask, feat, is_anomaly in zip(
                    _scores, _masks, _feats, data["is_anomaly"].numpy().tolist()
                ):
                    scores.append(score)
                    masks.append(mask)
