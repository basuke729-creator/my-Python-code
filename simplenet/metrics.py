...
def compute_imagewise_retrieval_metrics(anomaly_prediction_weights, anomaly_ground_truth_labels):
    """
    Computes retrieval statistics (AUROC, FPR, TPR, thresholds, PR-AUC).
    ...
    precision, recall, _thr = metrics.precision_recall_curve(labels, scores)
    auc_pr = metrics.auc(recall, precision)

    # Optional: best threshold by F1 (image-level)
    f1 = np.divide(
        2 * precision * recall,
        precision + recall,
    ...
    if len(f1) > 1 and len(_thr) > 0:
        best_idx = int(np.argmax(f1[:-1]))  # last point has no threshold
        if best_idx < len(_thr):
            best_threshold_f1 = float(_thr[best_idx])
...
def compute_pixelwise_retrieval_metrics(anomaly_segmentations, ground_truth_masks):
    ...
    Args:
        anomaly_segmentations: [list of np.arrays or np.array] [NxHxW] anomaly maps
        ground_truth_masks:    [list of np.arrays or np.array] [NxHxW] binary masks
    """
    anomaly_segmentations = _as_numpy(anomaly_segmentations)

    # If masks are missing / None-like
    ...
    # Guard: AUROC undefined if only one class in GT
    ...
    fpr, tpr, thresholds = metrics.roc_curve(flat_gt, flat_anomaly)
    auroc = metrics.roc_auc_score(flat_gt, flat_anomaly)

    precision, recall, pr_thresholds = metrics.precision_recall_curve(flat_gt, flat_anomaly)
    F1_scores = np.divide(
        2 * precision * recall,
        precision + recall,
        out=np.zeros_like(precision),
    ...
    optimal_threshold = pr_thresholds[best_idx] if len(pr_thresholds) > 0 else None
