...
def parse_args():
    ...
    ap.add_argument("--ckpt", required=True, help="train_effnetv2.py が出した best.ckpt のパス")
    ap.add_argument("--model", required=True, help="学習時に使った timm モデル名 (例: tf_efficientnetv2_m)")
    ap.add_argument("--img-size", type=int, default=384, help="入力画像サイズ (train と同じ値)")
    ap.add_argument("--out", default="model.onnx", help="出力する ONNX ファイルパス")
...
def main():
    ...
    torch.onnx.export(
        ...
        use_external_data_format=False,  # ★ 追加
    )
