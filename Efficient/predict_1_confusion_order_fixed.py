...
def build_label_space(y_true: List[str], y_pred: List[str], class_names: List[str], include_unknown: bool):
    # 指定した順番を優先して混同行列・評価結果のクラス順を固定する
    available = set(class_names) | set(y_true or []) | set(y_pred or [])

    labels = [
        label for label in CONFUSION_MATRIX_CLASS_ORDER
        if label in available
    ]

    # 指定順に含まれていないクラスが存在する場合は末尾へ追加
    remaining = [
        label for label in class_names
        if label not in labels and (include_unknown or label != "unknown")
    ]
    labels += remaining
    ...
    labels += [
        ...
        if (include_unknown or label != "unknown")
    ]
        ...
        if args.export_miscls:
            ...
            for p, t, pr in zip(paths_for_eval, y_true, y_pred):
                ...
                try:
                    ...
                    pair_dir = out_dir / f"true_{t}_pred_{pr}"
