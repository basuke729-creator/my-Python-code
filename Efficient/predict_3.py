...
# v1.7.1 + CM split outputs (44 full + 20 + 24)
# ※ 変更点：混同行列PNGを「44フル」「20クラス」「24クラス」で保存
...
print("predict.py v1.7.1 (speed YOLO-style + per-class summary + eval + CSV) + CM split outputs")
...
def plot_confusion_matrix_split(cm_full: np.ndarray, labels_full: List[str], out_dir: Path,
                                split_sizes=(20, 24), normalize=False, suffix=""):
    assert cm_full.shape[0] == cm_full.shape[1] == len(labels_full)

    start = 0
    for i, n in enumerate(split_sizes, 1):
        end = start + n
        sub_labels = labels_full[start:end]
        sub_cm = cm_full[start:end, start:end]

        out_png = out_dir / f"confusion_matrix_part{i}_{n}{suffix}.png"
        plot_confusion_matrix(sub_cm, sub_labels, out_png, normalize=normalize)
...
def main():
    ...
    # 速度計測のバッファ
    # YOLO側コードの出力に合わせて、preprocess / detect / postprocess / all で集計する
    prep_ms_list, infer_ms_list, post_ms_list, total_ms_list = [], [], [], []
    if args.speed and args.speed_csv:
        ...
        sp_w.writerow(["path", "preprocess_ms", "detect_ms", "postprocess_ms", "all_ms"])
    else:
        sp_f = None
        sp_w = None
    ...
    for img_path in iter_images(inp_root):
        ...
        # ---- 計測: preprocess + detect + postprocess + all ----
        # preprocess : 画像変換 + Tensor化 + GPU転送
        # detect     : EfficientNetV2のモデル推論
        # postprocess: softmax + topk + しきい値判定
        # all        : 上記すべてを含む1枚あたりの処理時間
        ...
        if args.tta and logits2 is not None:
    ...
    if args.speed and len(total_ms_list) > 0:
        count = len(total_ms_list)
        avg_prep = float(np.mean(prep_ms_list))
        avg_infer = float(np.mean(infer_ms_list))
        avg_post = float(np.mean(post_ms_list))
        avg_total = float(np.mean(total_ms_list))

        # 画像で送ってもらったYOLO側コードに近い出力形式
        print(
            "count %d, preprocess %f, detect %f, postprocess %f, all %f"
            % (count, avg_prep, avg_infer, avg_post, avg_total)
        )
        print(f"Processing time: {avg_total:.1f} [ms]")
        print(f"Inference time: {avg_infer:.1f} [ms]")
        print(f"FPS(all): {1000.0 / avg_total:.1f} [fps]" if avg_total > 0 else "FPS(all): 0.0 [fps]")
        print(f"FPS(detect): {1000.0 / avg_infer:.1f} [fps]" if avg_infer > 0 else "FPS(detect): 0.0 [fps]")
    ...
    if args.eval:
        ...
        # ★ PNG出力：44フル + 20/24分割（カウント & 正規化）
        plot_confusion_matrix(cm, labels, eval_dir / "confusion_matrix_full.png", normalize=False)
        plot_confusion_matrix(cm, labels, eval_dir / "confusion_matrix_full_norm.png", normalize=True)

        # 20/24分割：指定どおり
        if len(labels) >= (PART1_SIZE + PART2_SIZE):
            plot_confusion_matrix_split(cm, labels, eval_dir, split_sizes=(PART1_SIZE, PART2_SIZE), normalize=False, suffix="")
            plot_confusion_matrix_split(cm, labels, eval_dir, split_sizes=(PART1_SIZE, PART2_SIZE), normalize=True,  suffix="_norm")
        else:
            # もし44未満なら、前半20/残りで出す（安全策）
            part1 = min(PART1_SIZE, len(labels))
            part2 = max(0, len(labels) - part1)
            if part1 > 0:
                plot_confusion_matrix_split(cm, labels, eval_dir, split_sizes=(part1, part2), normalize=False, suffix="")
                plot_confusion_matrix_split(cm, labels, eval_dir, split_sizes=(part1, part2), normalize=True,  suffix="_norm")
