...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device):
    """検証：loss / macro precision / macro recall / macro f1 を返す"""
    model.eval()
    y_true, y_pred = [], []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (x, y) in enumerate(pbar, 1):
            ...
            if tb and global_step % 20 == 0:
                # 学習中は loss / LR を記録（Accuracyのログは残さない）
...
# ----------------- Args -----------------
def parse_args():
    ap = argparse.ArgumentParser("EfficientNet-V2 Transfer Learning (timm) – P/R/F1 TB logging")
